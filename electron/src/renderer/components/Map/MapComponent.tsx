...
import { IconHome, IconWorld, IconNumber1, IconNumber2, IconNumber3 } from "@tabler/icons-react";
...
import { setMyIp } from "renderer/globalStore";
...
const MapComponent = ({ h, scale }: { h: number; scale?: number }) => {
  ...
  // Add a small random offset to the coordinates to avoid overlap
    function addRandomOffset([lng, lat]: [number, number]) {
      const offset = () => (Math.random() - .95) * 9.5 // up to N deg offset
  ...
  const fetchMarkers = useCallback(async () => {
    ...
    const allMarkers = [
      ...
      ...ipLocations.map(({ location, ip, fingerprint, name }, index) => ({
        hop: index + 1,
        name,
        coordinates: addRandomOffset(location),
        //markerOffset: -20,
        ip,
        fingerprint,
      })),
